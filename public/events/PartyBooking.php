...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['action']) && $_POST['action'] === 'confirm_party_booking') {
    ...
    if (empty($customer_username)) {
        ...
        if (empty($customer_username)) {
            $customer_username = !empty($contact_email) ? explode('@', $contact_email)[0] : 'party_host';
        }
    }
                    ...
                    <div class="step-card">
                        ...
                        <div class="review-layout">
                            <div>
                                ...
                                <?php if (!$is_logged_in): ?>
                                    <div style="background:rgba(139, 92, 246, 0.08); border:1px solid rgba(139, 92, 246, 0.2); border-radius:14px; padding:18px; margin-top:20px;">
                                        <h5 style="color:var(--primary); font-weight:800; margin-bottom:6px;"><i class="fas fa-user-lock"></i> Quick Guest Booking</h5>
                                        <p style="color:var(--text-muted); font-size:13px; margin-bottom:12px;">
                                            You are booking as a guest. Enter a username to manage this party booking later, or sign in to link with your account.
                                        </p>
                                        <div class="form-group" style="margin-bottom:0;">
                                            <label class="form-label" for="guest_username">Preferred Username</label>
                                            <input type="text" class="form-input" id="guest_username" name="guest_username" placeholder="e.g. ravindu_party">
                                        </div>
                                    </div>
                                <?php endif; ?>
                            </div>
